...
def netOutput(net, inputs, outType="default", approach="default"):
  ...
  with torch.no_grad():
    for input1 in tqdm(inputs):
      if outType == "latent":
        output = net.latent(torch.from_numpy(input1).unsqueeze(0).float().to(device)).cpu().numpy()

      elif outType == "class":
        if approach == "fusion":
          input1 = [torch.from_numpy(ip).unsqueeze(0).float() for ip in input1]
          output = net(input1).cpu().numpy()
        else:
          output = net(torch.from_numpy(input1).unsqueeze(0).float().to(device)).cpu().numpy()
